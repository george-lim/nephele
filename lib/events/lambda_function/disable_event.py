...
def handler(event, _):
  ...
  if len(args) < 1:
    interface.reply_message('Please provide a scheduled event name to disable.')
  ...
  try:
    ...
    interface.reply_message('Successfully disabled scheduled event!')
  except:
    interface.reply_message('Failed to disable scheduled event: invalid event name.')
    raise
