...
from src.epic_games.functions.log_out import handler
...
def test_log_out(capsys, mock_users_table):
    ...
    assert out.strip() == "Successfully logged out of Epic Games!"
    assert err.strip() == ""
...
def test_log_out_exception(capsys, monkeypatch, mock_users_table):
    ...
    assert out.strip() == "Failed to log out of Epic Games."
    assert err.strip() == ""
